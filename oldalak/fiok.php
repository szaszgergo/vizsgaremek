...
if (isset($jegy)) {
    ...
    $maradek_napok = round((strtotime($jegy["jLejarat"]) - time()) / 86400);
    ...
    if (!is_null($jegy['jAlkalmak'])) {
        $szoveg .= "<h3>" . htmlspecialchars($languageContent["hasznalatok"]) . " <span class='gold'>$jegy[jAlkalmak]</span></h3>";
    }
} else {
    $szoveg = "
        ...
        <form class="profile-container" action="actions/fiok_adat_valtoztatas.php" method="post" target='kisablak'
            ...
            <div class="profile-header">
                <img style="object-fit: cover;"
                    src="<?php $a = 'profile_pic/' . (empty($adatok['uProfilePic']) ? '../images/pic.png' : $adatok['uProfilePic']);
                    echo $a; ?>"
                    alt="Profile Picture">
                <div>
            ...
            <div class="mb-3">
                ...
                <input value="<?php echo $adatok['uFelhasznalonev'] ?>" type="text" class="form-control fiok-input"
                    id="InputUsername" name="username" maxlength="100" readonly>
            </div>

            <div class="mb-3">
                <label for="InputRealName" class="form-label"><?= $languageContent["RealNameLabel"] ?></label>
                <input value="<?php echo $adatok['uSzuleteskorinev'] ?>" type="text" class="form-control fiok-input"
                    id="InputRealName" name="realname" maxlength="100" readonly>
            </div>


            <div class="mb-3">
                <label for="InputEmail" class="form-label"><?= $languageContent["emailLabel"] ?></label>
                <input value="<?php echo $adatok['uemail'] ?>" type="email" class="form-control fiok-input"
                    id="InputEmail" name="email" maxlength="256" readonly>
            </div>

            <div class="mb-3">
                <label for="InputDate" class="form-label"><?= $languageContent["birthdateLabel"] ?></label>
                <input value="<?php echo $adatok['uSzuletesidatum'] ?>" type="date" class="form-control fiok-input"
